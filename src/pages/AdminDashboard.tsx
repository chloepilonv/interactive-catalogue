...
export default function AdminDashboard() {
  ...
  // Check auth and fetch artifacts
  useEffect(() => {
  ...
  const saveArtifact = async () => {
    ...
    try {
      ...
      if (editingArtifact) {
        ...
        const { error } = await supabase.from('artifacts').insert(artifactData);
        if (error) throw error;
        toast.success('Artefact ajouté !');
      }
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="font-display text-lg font-semibold text-foreground">Gestion des artefacts</h1>
          <Button
            variant="ghost"
            size="sm"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {artifacts.map((artifact) => (
              ...
              >
                ...
                <div className="aspect-square bg-muted relative">
                  ...
                  {artifact.photos && artifact.photos.length > 1 && (
                    <div className="absolute bottom-2 right-2 bg-black/70 text-white text-xs px-2 py-1 rounded-full">
                      +{artifact.photos.length - 1} photos
                    </div>
                  )}
                </div>
                ...
                {/* Info */}
                <div className="p-4">
                  <h3 className="font-semibold text-foreground line-clamp-1">{artifact.name}</h3>
                  <p className="text-sm text-muted-foreground line-clamp-1 mt-1">
                    {artifact.date || 'Date inconnue'}
                  </p>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingArtifact ? 'Modifier l\'artefact' : 'Ajouter un artefact'}
            </DialogTitle>
          </DialogHeader>
            ...
            <Button onClick={saveArtifact} disabled={isSaving || !form.name.trim()}>
              {isSaving ? (
                ...
                <>
                  ...
                  {editingArtifact ? 'Mettre à jour' : 'Enregistrer'}
                </>
              )}
            </Button>
          </div>
